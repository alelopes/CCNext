...
#         ccnext_enc = ConvnextCAEncoder(sizes, device, 0.26)
...
#         import sys
#         sys.path.append("../stereo_depth/monodepth2-2/")
#         import hnet
        
#         num_ch_enc = np.array([64, 64, 128, 256, 512])
#         decoder_mode = "SiamResMHADecoder"
#         siam_encoder = hnet.ConvnextMHAEncoder_2_fixed2_wd(18, True, sizes, device, num_input_images=1)

#         middle = hnet.Middle(decoder_mode=decoder_mode)
#         num_ch_enc_decoder = np.array([32, 64, 128, 256, 512])
#         siam_decoder = hnet.DecoderSecondApproach(num_ch_enc=num_ch_enc_decoder, is_c_equal=False)
#         training_parameters = []
#         training_parameters += list(siam_encoder.parameters())
#         training_parameters += list(middle.parameters())
#         training_parameters += list(siam_decoder.parameters())
...
#         self.enc = siam_encoder.to(device)
#         self.middle =  middle.to(device)
#         self.dec = siam_decoder.to(device)
    ...
    def train(self):
        print('will trainnnnnnnnnnnnnnnnnn')
        self.enc.train()
#         self.middle.train()
        self.dec.train()
...
#                 print('do flip', batch['do_flip'])
#                 out_fist = self.enc(batch["color_aug", 0, 0], batch["color_aug", "s", 0], batch['do_flip'])
#                 mid = self.middle(out_fist)
#                 outputs = self.dec(mid)
                out_fist = self.enc(batch["color_aug", 0, 0], batch["color_aug", "s", 0])
                outputs = self.dec(out_fist)


                outputs[('disp', 3)] = outputs[('disp0', 3)]
                outputs[('disp_s', 3)] = outputs[('disp1', 3)]
                outputs[('disp', 2)] = outputs[('disp0', 2)]
                outputs[('disp_s', 2)] = outputs[('disp1', 2)]
                outputs[('disp', 1)] = outputs[('disp0', 1)]
                outputs[('disp_s', 1)] = outputs[('disp1', 1)]
                outputs[('disp', 0)] = outputs[('disp0', 0)]
                outputs[('disp_s', 0)] = outputs[('disp1', 0)]     
...
#         self.middle.eval()
        ...
        with tqdm(self.val_loader, unit="batch") as tepoch:
            ...
            for idx, batch in enumerate(tepoch):                    
                ...
                with torch.no_grad():
                    out_fist = self.enc(batch["color", 0, 0], batch["color", "s", 0])#, batch['do_flip'])
#                     mid = self.middle(out_fist)
                    outputs = self.dec(out_fist)

#                     out_fist = self.enc(batch["color", 0, 0], batch["color", "s", 0])
#                     outputs = self.dec(out_fist)
    ...
    def save_model(self):
        ...
        """
        save_folder = os.path.join("models_3", "weights_{}".format(self.epoch))
...
#         save_path = os.path.join(save_folder, f"middle.pth")
#         to_save = self.middle.state_dict()
#         torch.save(to_save, save_path)        
...
def parse_args():
    ...
    parser.add_argument("--height", type=int, help="Training height", default=192)
    parser.add_argument("--width", type=int, help="Training width", default=640)
